...
func (info *HalsteadInfo) addToken(nextToken token.Token) {
	tokenName := fmt.Sprintf("token:%d", nextToken)
...
func (info *HalsteadInfo) AddCommentMap(node ast.CommentMap) {
	// Nothing to count
	return
}
...
func (info *HalsteadInfo) AddObject(node ast.Object) {
	// Nothing to count
	return
}
...
// TODO: double check all tokens for being handled
func (info *HalsteadInfo) AddReturnStmt(node ast.ReturnStmt) {
	info.addToken(token.RETURN)
}

func (info *HalsteadInfo) AddScope(node ast.Scope) {
	// Something abstract, ignore
	return
}
...
func (info *HalsteadInfo) AddSelectorExpr(node ast.SelectorExpr) {
	// Something composite only (everything will be walked through), ignore
	return
}
